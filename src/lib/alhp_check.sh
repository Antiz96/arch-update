...
if [ -n "${alhp_support}" ]; then
    alhp_utils_output="$(${alhp_support} -m)"
    alhp_utils_exit_code=$?

    # Exit code 20 is be returned if the primary ALHP mirror is out of date.
    # Other exit codes return other errors that we should show to the user.
    # No exit code, or exit code 20, means there are queued packages.
